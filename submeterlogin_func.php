...
$tipo = $_REQUEST['tipo'];
$query=("SELECT * FROM $tipo WHERE Username='".$username."'");
$query2=("SELECT Password FROM $tipo WHERE Username='".$username."'");
$query3=("SELECT * FROM $tipo WHERE Username='".$username."' AND Password='".$password."'");
$users=mysql_query($query);
$pass=mysql_query($query2);
while ($pw=mysql_fetch_array($pass))
{
$pwd=$pw['Password'];
}
$conta=mysql_query($query3);
...
{
	echo("<p>O username <b>$username</b> não existe. Por favor, insira outro.</a></p>");
	echo "<a href='javascript:history.back()' class='backLink' title='Voltar atr&aacute;s'>Voltar atr&aacute;s</a><br />";
	echo "<img src='imgs/aviso.png' height='230' width='230'/>";
}
...
else if ((mysql_num_rows($conta)>0))		
{ 
	$_SESSION['Username']=$username;
	if ($tipo=="Gerentes")
	{
		$_SESSION['TipoUtilizador']="Gerente";
		echo("<script>location.href = 'gerir_encomendas.php';</script>");
	}
	else 
	{
		$_SESSION['TipoUtilizador']="Lojista";
