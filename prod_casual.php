        	...
        	<div class="sidebar_box"><span class="bottom"></span>
                ...
                <div class="content"> 
                	<ul class="sidebar_list">
                    	<li class="first"><a href="prod_casual.php"><b>Casual</b></a></li>
...
<h1>Lista de Produtos - Casual</h1>
<div align="center">
<?php
include "conn.php";
    $con = mysqli_connect($hostname_conn,$username_conn,$password_conn,$database_conn);
// Função para fazer as colunas dos produtos.
function GeraColunas($con, $pNumColunas, $pQuery) {
$resultado = mysqli_query($con,$pQuery); //Executa a query guardada em $sql e guarda o resultado em $resultado.
...
if (isset($_REQUEST['genero']) && $_REQUEST['genero']=="senhora")
{ 
	?>
	<script>checksenhora()</script>
	<?php
	if (isset($_REQUEST['ordenar']) && $_REQUEST['ordenar']=="preco_asc")
	{
		?>
		...
		<?php
		$sql = 'SELECT * FROM produtos WHERE Genero="Senhora" AND Tipo="Casual" ORDER BY Preco ASC';
	...
	else if (isset($_REQUEST['ordenar']) && $_REQUEST['ordenar']=="preco_desc")
	{ 
		?>
		<table cellSpacing=1 cellPadding=0 width="50%" align=center border=0>
		<tr>
		...
		<?php
		$sql = 'SELECT * FROM produtos WHERE Genero="Senhora" AND Tipo="Casual" ORDER BY Preco DESC';
		GeraColunas($conn, 3, $sql)
		?>
		</td>
		</tr>
	...
	else
	{
		...
		<script>checksenhora()</script>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Genero="Senhora" AND Tipo="Casual" ORDER BY RAND()';
		GeraColunas($conn, 3,$sql)
		?>
		</td>
		</tr>
...
else if (isset($_REQUEST['genero']) && $_REQUEST['genero']=="homem")
{ 
	?>
	...
	if (isset($_REQUEST['ordenar']) && $_REQUEST['ordenar']=="preco_asc")
	{
		...
		<script>checkprecoasc()</script>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Genero="Homem" AND Tipo="Casual" ORDER BY Preco ASC';
		GeraColunas($conn,3, $sql)
		?>
		</td>
		</tr>
	...
	{ 
		...
		<script>checkprecodesc()</script>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Genero="Homem" AND Tipo="Casual" ORDER BY Preco DESC';
		GeraColunas($conn,3, $sql)
		?>
		</td>
		</tr>
	...
	else
	{
		...
		<td>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Genero="Homem" AND Tipo="Casual" ORDER BY RAND()';
		GeraColunas($conn,3,$sql)
		?>
		</td>
		</tr>
...
{ 
	...
	if (isset($_REQUEST['ordenar']) && $_REQUEST['ordenar']=="preco_asc")
	{
		...
		<script>checkprecoasc()</script>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Tipo="Casual" ORDER BY Preco ASC';
		GeraColunas($conn,3, $sql)
		?>
		</td>
		</tr>
	...
	{ 
		...
		<script>checkprecodesc()</script>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Tipo="Casual" ORDER BY Preco DESC';
		GeraColunas($conn,3, $sql)
		?>
		</td>
		</tr>
	...
	else
	{
		...
		<td>
		<?php
		$sql = 'SELECT * FROM produtos WHERE Tipo="Casual" ORDER BY RAND()';
		GeraColunas($conn,3,$sql)
		?>
		</td>
		</tr>
